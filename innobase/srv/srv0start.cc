...
static lsn_t srv_prepare_to_delete_redo_log_files(ulint n_files) {
  ...
  do {
    ...
    for (ulint i = 0; i < srv_buf_pool_instances; i++) {
      ...
      while (true) {
        sync_file_range(p->file, p->offset, p->len, SYNC_FILE_RANGE_WRITE);
